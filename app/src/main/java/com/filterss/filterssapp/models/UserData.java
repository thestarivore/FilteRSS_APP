...
public class UserData {
    ...
    public void processMultifeedMap(){
        multifeedMap.clear();
        List<Integer> selectedFeed = new ArrayList<>();
        ...
        for(Multifeed multifeed: multifeedList){
            ...
            for(FeedGrouping feedGroup: feedGroupList){
                if(feedGroup.getMultifeed() == multifeedId){
                    mfeedList.add(getFeedById(feedGroup.getFeed(), selectedFeed));
                }
            }
     ...
     */
    private Feed getFeedById(int feedId, List<Integer> selectedFeeds) {
        int skip = 0;
        for(Integer selectedFeed : selectedFeeds){
